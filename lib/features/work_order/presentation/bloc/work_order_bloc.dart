/*
Tujuan: BLoC Work Order untuk list, detail, create, approval, reject, dan extension.
Caller: WorkOrderPage, WoCreatePage, WoDetailPage.
Dependensi: flutter_bloc, WorkOrderRepository, WorkOrderEvent, WorkOrderState.
Main Functions: _onLoad, _onLoadDetail, _onCreate, _onApprove, _onReject.
Side Effects: Dispatch request repository dan emit state UI Work Order.
*/
...
class WorkOrderBloc extends Bloc<WorkOrderEvent, WorkOrderState> {
  ...
  Future<void> _onLoadDetail(
    ...
    result.fold(
      (f) =>
          emit(WorkOrderError(message: f.message ?? 'Gagal memuat detail WO')),
  ...
  Future<void> _onCreate(
    ...
    await result.fold(
      (f) async =>
          emit(WorkOrderError(message: f.message ?? 'Gagal membuat WO')),
  ...
  Future<void> _onApprove(ApproveWo e, Emitter<WorkOrderState> emit) async {
    ...
    await result.fold(
      (f) async =>
          emit(WorkOrderError(message: f.message ?? 'Gagal approve WO')),
      (data) async {
        final nextStage = data['newStage']?.toString() ?? '';
        final msg = nextStage == 'APPROVED'
            ? 'WO disetujui final ✓'
            : 'Diteruskan ke $nextStage';
  ...
  Future<void> _onReject(RejectWo e, Emitter<WorkOrderState> emit) async {
    ...
    await result.fold(
      (f) async =>
          emit(WorkOrderError(message: f.message ?? 'Gagal reject WO')),
      (_) async => _refreshAfter(emit, 'WO berhasil ditolak'),
    );
  }

  Future<void> _onRequestDl(
    ...
    await result.fold(
      (f) async => emit(
        WorkOrderError(message: f.message ?? 'Gagal ajukan perpanjangan DL'),
      ),
      (_) async =>
          _refreshAfter(emit, 'Pengajuan perpanjangan deadline dikirim'),
    );
  }

  Future<void> _onRespondDl(
    ...
    await result.fold(
      (f) async => emit(
        WorkOrderError(message: f.message ?? 'Gagal proses perpanjangan DL'),
      ),
      (_) async => _refreshAfter(
        emit,
        e.approve ? 'Perpanjangan DL disetujui' : 'Perpanjangan DL ditolak',
      ),
    );
  }

  Future<void> _onRequestHours(
    ...
    await result.fold(
      (f) async => emit(
        WorkOrderError(message: f.message ?? 'Gagal ajukan tambahan jam'),
      ),
      (_) async => _refreshAfter(emit, 'Pengajuan tambahan jam dikirim'),
    );
  }

  Future<void> _onRespondHours(
    ...
    await result.fold(
      (f) async => emit(
        WorkOrderError(message: f.message ?? 'Gagal proses tambahan jam'),
      ),
      (_) async => _refreshAfter(
        emit,
        e.approve ? 'Tambahan jam disetujui' : 'Tambahan jam ditolak',
      ),
    );
  }
}
