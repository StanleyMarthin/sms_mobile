...
class _PrDetailPageState extends State<PrDetailPage> {
  ...
  bool _canApprove() {
    ...
    return switch (acc) {
      ...
      'PENDING_PUR' =>
        !session.isFieldExecution && session.hasPerm(Perms.prApprove),
      _ => false,
    };
  }
...
class _HeaderCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final (stageLabel, stageColor) = _stageInfo(
      header.accTracking,
      header.status,
    );
              ...
              if (header.createdAt != null)
                _chip(
                  ...
                  DateFormat(
                    'd MMM yyyy',
                    'id_ID',
                  ).format(header.createdAt!.toLocal()),
                ),
          ...
          child: Row(
            children: [
              ...
              if (header.huntingItems > 0)
                _mini('  ${header.huntingItems} hunting', AppColors.gold),
              if (header.orderedItems > 0)
                _mini('  ${header.orderedItems} ordered', AppColors.orange),
            ],
          ),
        ),
        ...header.items.map(
          (item) =>
              _ItemCard(item: item, isActing: isActing, onFinalize: onFinalize),
        ),
      ],
    );
  }
