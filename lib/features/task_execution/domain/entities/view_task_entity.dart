// View Task Entity — domain entity for the unified tasks view.
//
// Maps to the GET /api/v1/tasks response structure.
// Contains nested value objects for division, unit, employee, and task details.
library;
...
class TaskCheckpointSession extends Equatable {
  ...
  String get reviewersLabel => reviewers.map((item) => item.roleLabel).join(' dan ');

  String get reviewerNamesLabel => reviewers.map((item) => item.name).join(', ');
...
class ViewTaskEntity extends Equatable {
  ...
  bool get isSubmitted => status == 'SUBMITTED';
    ...
    bool hasFinalValidationByRole(String role) =>
      finalValidations.any((item) => item.role.toLowerCase() == role.toLowerCase());
  ...
  bool get isCheckpointFlowFinished =>
      checkpointHistory.isNotEmpty && checkpointHistory.last.jobStatus == 'DONE';

  bool get hasRemainingCheckpointSessions =>
      !isCheckpointFlowFinished && checkpointHistory.length < maxCheckpointSessions;
