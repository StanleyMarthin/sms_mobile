...
/plans → JobPlanPage create sheet / legacy /plans/create redirect
-> _CountdownPlanFormPage memakai CountdownRepository untuk Unit -> Panel -> Jobdesc
...
→ Create WOV dari Tracking memakai POST /sm/wov action=create dengan `masterPanelId` wajib; `coreId` opsional sebagai konteks Countdown terkait
...
→ Operational counts clean: sm_jobdesc_countdown.panel_id=master_panels.id, sm_jobdesc_plan.core_id→countdown.panel_id, pur_pr_header.panel_id, sm_jobdesc_wo.master_panel_id, vnd_wo_vendor.master_panel_id; fallback legacy WOV via pr_id/core_id hanya untuk read
